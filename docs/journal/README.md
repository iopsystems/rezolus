...
| 2026-07-15 | [Measurement uncertainty — `.rez` reader ecosystem (viewer / MCP / parquet-tools)](2026-07-15-rez-reader-ecosystem.md) | In progress — Phase A + **Phase B core landed**. Sub-project (3): make `.rez` readable everywhere `.parquet` is. Union `RezReader: MetricsSource` (N per-sampler readers, clear error on cross-table). A (metriken-query skips `:window_*` sidecars) ✓; B (viewer file-mode + MCP + `parquet metadata`) ✓ — upload-mode + Prometheus guard deferred; C (multi-recording assembly + label faceting + `combine`/`filter`/`annotate`) pending |
