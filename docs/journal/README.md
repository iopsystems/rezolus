...
| 2026-08-11 | [Streaming segmented `.rez` writer — bounded-time finalization](2026-08-11-rez-streaming-writer.md) | OPEN — design landed pre-build. Driver: docker tear-down (~10 s SIGTERM→SIGKILL) vs finalize cost that scales with recording length. Design: seal per-sampler parquet segments into the output tar during recording (`<dir>/<sampler>/<seq>.parquet`, ~512 rows / ~5 min), checkpoint `manifest.json` after every seal (kill-safe: readable up to last sealed segment), finalize = seal tails + manifest + footer; manifest v2 `files: Vec<String>` (v1 readable); segment merge + truncation tolerance centralized in `read_archive_reader`; drops the `.rez`-mode msgpack spool double-write |
