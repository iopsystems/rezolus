...
| 2026-07-16 | [Measurement uncertainty — histogram value uncertainty (bucket resolution)](2026-07-16-histogram-value-uncertainty.md) | Open — design landed, pre-build. Follow-on to sub-project (4): `histogram_quantile` gets a **value** band = the containing bucket `[start, end]` (from `grouping_power`/`max_value_power`, already in the parquet — no windows). Needs `(Materialized, Scalar)` binary support so `histogram_quantiles(...)/1e9` latency panels keep the band. Reuses the `QueryResult.intervals`/MCP/viewer infra. `histogram_sum`/`mean` deferred |
