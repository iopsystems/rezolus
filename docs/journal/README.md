...
| 2026-07-13 | [Measurement uncertainty — per-sampler `.rez` archive (sampler grouping + recorder)](2026-07-13-per-sampler-rez-archive.md) | IMPLEMENTED & VALIDATED (sub-projects 1+2), format revision pending. Per-sampler tables in a `.rez` tar (windows first-class, window-advance dedup); module-path attribution resurrected as a `sampler` label; live-smoke validated (25 tables, drivehealth 1 row vs fast 7). Shakedown moved the container to a **label-set model** (bag of label-tagged recordings) — flat-build→label-set delta reviewed and journaled; per-metric parquet unchanged. First of 4 sub-projects to make windows usable (readers + query/rate-error-bars follow) |
