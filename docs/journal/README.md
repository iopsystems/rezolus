...
| 2026-07-15 | [Measurement uncertainty — rate() error bars (query-engine leaf)](2026-07-15-rate-error-bars.md) | Open — design landed, pre-build. Sub-project (4), the arc's culmination: `rate()`/`irate()` turn per-observation windows into honest uncertainty via interval arithmetic (`rate ∈ [Δv/(e_last−b_first), Δv/(b_last−e_first)]`). Leaf-only (bounds drop under operators); `QueryResult` carries intervals; MCP `query` surfaces them. Tier-1 propagation, viewer bands, and correlation ceiling deferred to later rounds |
