...
impl MmapInner {
    ...
    pub fn map(len: usize, file: RawFd, offset: u64, populate: bool, fixed: bool) -> io::Result<MmapInner> {
        let populate = if populate { MAP_POPULATE } else { 0 };
        let fixed = if fixed { MAP_FIXED } else { 0 };
        MmapInner::new(
            len,
            libc::PROT_READ,
            libc::MAP_SHARED | populate | fixed,
            file,
            offset,
        )
    }
    ...
    pub fn map_mut(len: usize, file: RawFd, offset: u64, populate: bool, fixed: bool) -> io::Result<MmapInner> {
        let populate = if populate { MAP_POPULATE } else { 0 };
        let fixed = if fixed { MAP_FIXED } else { 0 };
        MmapInner::new(
            len,
