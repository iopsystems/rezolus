...
impl MmapOptions {
    ...
    pub unsafe fn map<T: MmapAsRawDesc>(&self, file: T) -> Result<Mmap> {
        ...
        MmapInner::map(self.get_len(&file)?, desc.0, self.offset, self.populate, self.fixed)
    ...
    pub unsafe fn map_mut<T: MmapAsRawDesc>(&self, file: T) -> Result<MmapMut> {
        ...
        MmapInner::map_mut(self.get_len(&file)?, desc.0, self.offset, self.populate, self.fixed)
