...
const TIME_LAGS: &[i32] = &[
    ...
    1, 2, 3, 5, 10,
    // Short-term effects (every 5s)
    15, 20, 25, 30,
    // Medium-term effects (every 10s)  
    40, 50, 60,
    // Longer-term effects (every 30s)
...
fn calculate_correlation_at_lag(
    data1: &[f64],
    data2: &[f64],
    lag: i64,
) -> Option<(f64, usize)> {
...
pub fn format_correlation_result(result: &CorrelationResult) -> String {
    ...
    if !result.series_pairs.is_empty() {
        ...
        output.push_str(&format!(
            ...
            if result.series_pairs.len() == 1 { "" } else { "s" }
        ));
    ...
    output.push_str("Positive lag means metric 2 follows metric 1 (metric 1 is leading indicator).\n");
