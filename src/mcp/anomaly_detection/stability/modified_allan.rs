...
/// Detect changes in noise characteristics using sliding window Modified Allan analysis
fn detect_modified_allan_transitions(
    values: &[f64],
    sample_interval: f64,
    ...
    while start + window_size <= values.len() {
        ...
        if let Some(tau_result) = modified.get(tau_samples) {
