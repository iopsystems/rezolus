...
pub struct HadamardAnalysis {
    ...
    pub noise_transitions: Vec<NoiseTransition>,
}

pub(in crate::mcp::anomaly_detection) fn perform_hadamard_analysis(
    values: &[f64],
    sample_interval: f64,
...
/// Detect changes in noise characteristics using sliding window Hadamard analysis
fn detect_hadamard_transitions(
    values: &[f64],
    sample_interval: f64,
    ...
    while start + window_size <= values.len() {
        ...
        // Compute Hadamard deviation for this window
        let mut hadamard = Hadamard::new();
        for &value in window {
            hadamard.record(value);
        }
        ...
        if let Some(tau_result) = hadamard.get(tau_samples) {
