...
impl ProcStat {
	pub fn new(_config: &Config) -> Self {
		...
		let counters = vec![
            (Counter::new(&CPU_USAGE_USER, Some(&CPU_USAGE_USER_HEATMAP)), 1),
            (Counter::new(&CPU_USAGE_NICE, Some(&CPU_USAGE_NICE_HEATMAP)), 2),
            (Counter::new(&CPU_USAGE_SYSTEM, Some(&CPU_USAGE_SYSTEM_HEATMAP)), 3),
            (Counter::new(&CPU_USAGE_IDLE, Some(&CPU_USAGE_IDLE_HEATMAP)), 4),
            (Counter::new(&CPU_USAGE_IO_WAIT, Some(&CPU_USAGE_IO_WAIT_HEATMAP)), 5),
            (Counter::new(&CPU_USAGE_IRQ, Some(&CPU_USAGE_IRQ_HEATMAP)), 6),
            (Counter::new(&CPU_USAGE_SOFTIRQ, Some(&CPU_USAGE_SOFTIRQ_HEATMAP)), 7),
            (Counter::new(&CPU_USAGE_STEAL, Some(&CPU_USAGE_STEAL_HEATMAP)), 8),
            (Counter::new(&CPU_USAGE_GUEST, Some(&CPU_USAGE_GUEST_HEATMAP)), 9),
            (Counter::new(&CPU_USAGE_GUEST_NICE, Some(&CPU_USAGE_GUEST_NICE_HEATMAP)), 10),
        ];

        let nanos_per_tick = 1_000_000_000 / (sysconf::raw::sysconf(sysconf::raw::SysconfVariable::ScClkTck)
