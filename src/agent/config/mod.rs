...
const OPT_IN_SAMPLERS: &[&str] = &["gpu_amd_pmu", "sensors"];
...
impl Config {
    ...
    /// `filesystem`, GPU PMUs and `sensors`).
    #[cfg_attr(not(target_os = "linux"), allow(dead_code))]
...
mod tests {
    ...
    fn opt_in_sampler_on_only_when_explicitly_enabled() {
        let c = config("[defaults]\nenabled = true\n\n[samplers.gpu_amd_pmu]\nenabled = true\n");
        assert!(c.enabled("gpu_amd_pmu"));
        let c = config("[samplers.sensors]\nenabled = true\ninterval = \"7s\"\n");
        assert!(c.enabled("sensors"));
        assert_eq!(c.sampler_interval("sensors"), Some(Duration::from_secs(7)));
    }
