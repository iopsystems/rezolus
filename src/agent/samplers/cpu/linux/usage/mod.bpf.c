    ...
    *last_time = curr_time;
    ...
    if (bpf_core_field_exists(task->sched_task_group)) {
        ...
        if (cgroup_id && cgroup_id < MAX_CGROUPS) {
            ...
            // we check to see if this is a new cgroup by checking the serial
            // number

            elem = bpf_map_lookup_elem(&cgroup_serial_numbers, &cgroup_id);

            if (elem && *elem != serial_nr) {
                // zero the counters, they will not be exported until they are
                // non-zero
                u64 zero = 0;
                ...
                bpf_map_update_elem(&cgroup_serial_numbers, &cgroup_id,
                                    &serial_nr, BPF_ANY);
            }
...
// The kprobe handler is not always invoked, so using the delta to count the CPU
// usage could cause undercounting. Kernel increases the task utime/stime before
// invoking cpuacct_account_field. So we count the CPU usage by tracking the
// per-task utime/stime. The user time includes both the CPUTIME_NICE and
// CPUTIME_USER. The system time includes CPUTIME_SYSTEM, CPUTIME_SOFTIRQ and
// CPUTIME_IRQ.
SEC("kprobe/cpuacct_account_field")
int BPF_KPROBE(cpuacct_account_field_kprobe, struct task_struct *task,
               u32 index, u64 delta)
{
