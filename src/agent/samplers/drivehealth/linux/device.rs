...
///
/// No per-drive acquisition window: the whole sweep (this function's caller,
/// `read_all`, plus the per-drive `set()` loop that follows it in
/// `linux/mod.rs`) is bracketed by ONE `AcquisitionGroup`
/// (`stats::DRIVEHEALTH_SWEEP_ACQ`) — see its doc comment for why per-drive
/// windows retired in favor of one group covering the sweep.
...
fn read_one(drive: &Drive) -> DriveReading {
    match drive.drive_type {
        ...
        DriveType::Sata => {
            let temperature_c = super::ata::read_temperature(&drive.node);
            DriveReading {
                temperature_c,
                nvme: None,
            }
