//! AMD GPU performance-level control, shared by the `gpu_amd_pmu` sampler and
//! the recorder.
//!
...
/// Apply `level` to specific GPU indices via `amd-smi set -g <idx> -l <level>`.
/// Indices are the amd-smi GPU indices (`-g N`), which match the rocprofiler
/// agent enumeration order Rezolus uses elsewhere.
///
/// Best-effort: a failure on one GPU is logged and recording/sampling continues
/// (the counters that need the stable state will simply read zero). Returns the
/// number of GPUs successfully set.
pub fn apply(level: PerfLevel, gpu_indices: &[usize]) -> usize {
    ...
    for &idx in gpu_indices {
        match run_amd_smi(level, Some(idx)) {
            Ok(()) => {
                crate::debug!(
                    "gpu_amd_pmu: set GPU {idx} perf level to {}",
                    level.as_str()
                );
                ok += 1;
            }
            Err(e) => crate::warn!(
                "gpu_amd_pmu: failed to set GPU {idx} perf level to {}: {e}",
                level.as_str()
            ),
        }
    }
...
/// Apply `level` to **all** GPUs via `amd-smi set -l <level>` (no `-g`). Used by
/// the always-on sampler, which sets the state before rocprofiler init — before
/// the GPU count is known. Returns `true` on success.
pub fn apply_all(level: PerfLevel) -> bool {
    if reject_determinism(level) {
        return false;
    }
    match run_amd_smi(level, None) {
        Ok(()) => {
...
/// Run `amd-smi set [-g <idx>] -l <level>`. `idx = None` targets all GPUs.
fn run_amd_smi(level: PerfLevel, idx: Option<usize>) -> Result<(), String> {
