...
#[metric(
    ...
    metadata = { sampler = "blockio_errors"}
)]
...
#[metric(
    ...
    metadata = { unit = "nanoseconds", sampler = "blockio_errors"}
)]
 ...
 * because the driver couldn't complete it (SCSI EH escalation, NVMe
 * controller reset, multipath path failover). These are recovered
 * events, distinct from terminal errors above.
 */
