...
- **Main Layout Structure**:
  ...
  * Optional status footer showing loading progress and any warnings/errors
...
- **Global Time Controls**:
  * Timeline overview showing the entire recording duration
  * Brush selection for time window zooming
  * Preset time window options (last 5m, 15m, 1h, 6h, etc.)
  * Skip forward/backward buttons
  * Reset to view entire recording

- **Time Cursor**:
  * Vertical line showing current point of interest
  * Synchronizes across all visible charts
  * Displays values at cursor position for all visible metrics
...
- **Chart Features**:
  ...
  * Automatic Y-axis scaling with manual override option
...
- **Chart Interactions**:
  ...
  * Click: Select specific timestamp (synchronizes across all charts)
...
- **Backend**:
  ...
  * HTML generation using templating
...
3. Backend generates HTML with embedded data and uPlot config
4. Browser renders the dashboard
5. Interactions trigger Rust backend processing as needed
6. Results update the UI via standard form submissions or AJAX
...
- **Primary Navigation**:
  ...
  * Breadcrumb navigation for drill-down flows
  * Tab navigation within subsystem views
...
- **Performance Testing**:
  * Load testing with large recordings
