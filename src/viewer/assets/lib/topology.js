...
// ── Compare-mode systeminfo diff banner ─────────────────────────────

const diffSysinfoFields = (a, b) => {
    if (!a || !b) return [];
    const out = [];
    ...
    for (const k of keys) {
        try {
            if (JSON.stringify(a[k]) !== JSON.stringify(b[k])) {
                out.push({ key: k, baseline: a[k], experiment: b[k] });
            }
        } catch (_) {
            // Fall back to reference equality if stringify fails (e.g. cyclic).
            if (a[k] !== b[k]) {
                out.push({ key: k, baseline: a[k], experiment: b[k] });
            }
        }
...
const CompareBanner = {
    view({ attrs }) {
        if (!attrs.compareMode) return null;
        const diffs = diffSysinfoFields(attrs.baselineSysinfo, attrs.experimentSysinfo);
        if (diffs.length === 0) return null;
        return m('div.compare-banner', [
            m('strong', 'Systeminfo differs:'),
            m('ul.compare-banner-list', diffs.map((d) =>
                m('li', [
                    m('code', d.key),
                    ': ',
                    m('span.compare-baseline', formatSysinfoValue(d.baseline)),
                    m('span.compare-arrow', ' \u2192 '),
                    m('span.compare-experiment', formatSysinfoValue(d.experiment)),
                ]))),
        ]);
    },
};

export { CpuTopology, CompareBanner };
