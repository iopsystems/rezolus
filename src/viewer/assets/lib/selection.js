...
const entriesFromPayload = (entries) => (entries || []).map(e => ({
    ...
    // Category-bridge fields are optional on older payloads — default
    // to null so a v3 export pinned before this fix degrades to
    // single-query compare (same behavior as before the field existed).
...
const toggleSelection = (spec, sectionKey, sectionName, groupName, compareMeta = null) => {
    ...
    notebookStore.entries.push({
        ...
        // Category-bridge fields: when this chart's section runs
        // different per-side queries (a category KPI generated by
        // crates/dashboard/src/dashboard/category.rs), we must
        // preserve the experiment-side query AND the member-name
        // set on the pinned entry. Without these, the Notebook
        // re-renders the chart in compare mode using only the
        // baseline query — which returns no data on the experiment
        // capture (different metric name) — and the user sees only
        // the baseline trace.
    ...
    async _loadCharts() {
        const promises = store.entries.map(async (entry) => {
            ...
            const spec = {
                ...
                // Preserve the per-side experiment query for category-
                // bridge KPIs. Without this, NotebookView's compare-mode
                // render runs the baseline query on the experiment side
                // — wrong metric — and only baseline data shows up.
            ...
            notebookStore.entries.map((entry) => {
                ...
                const chartBody = (attrs.compareMode && spec.promql_query)
                    ? m(CompareChartWrapper, {
                        ...
                        captureLabels,
                        // Per-entry category bridge (set on pin from the
                        // section's metadata). Lets composeScatterLabel
                        // drop capture-identity dims for bridge KPIs
                        // without depending on the live Notebook section's
                        // metadata, which doesn't carry category_members.
