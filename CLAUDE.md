...
3. **Recorder** (`src/recorder/`) - Writes metrics to disk. Auto-detects Rezolus vs Prometheus sources. The output extension picks the format (`.rez` default, `.parquet`, `.raw`); `--format` is the explicit form and contradicting the extension is an error. Defaults to `rezolus.rez` when no `-o` is given; neither the source (Prometheus records into a `.rez` as one acquisition group per target) nor the endpoint count changes the format, only `--separate` does. Several rezolus endpoints in one run write one multi-recording `.rez`, a recording each. Supports `--metadata key=value`; `--label key=value` tags every `.rez` recording in the run, so `--endpoint url,source=name` is what distinguishes two endpoints (see "`.rez` archive format" below).
...
**The format specification is `docs/rez-format.md`** — container, catalog, segment and WAL encodings, reserved metadata keys, time model, and what bumps the version. Read it before changing anything a reader interprets.
