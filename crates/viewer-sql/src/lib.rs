...
use prom_matrix::EMPTY_PROM_MATRIX;
...
impl ViewerSql {
    ...
    pub async fn query_range(
        ...
        match self.query(&wrapped).await {
            Ok(table) => prom_matrix::js_arrow_to_prom_matrix(&table),
            Err(e) => {
...
// Arrow → Prometheus matrix projection lives in `prom-matrix` (the
// `js_arrow_to_prom_matrix` entry point on wasm32). Kept out of this
// crate so the native server-backed viewer shares the same JSON shape
